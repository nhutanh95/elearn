...
const PopUpCancelLesson = ({ LessionName, date, start, end, reason }) => {
  ...
  return <div className="modal fade effect-scale" id="md-cancel-schedule-popup" tabIndex="-1" role="dialog" aria-labelledby="active-slot"
    aria-hidden="true">
    <div className="modal-dialog modal-dialog-centered modal-md" role="document">
      <div className="modal-content">
        <div className="modal-header bg-gray-600">
          <h5 className="modal-title tx-white">Buổi học của bạn đã được hủy</h5>
          <button type="button" className="close" data-dismiss="modal" aria-label="Close">
            <span className="tx-white" aria-hidden="true">&times;</span>
          </button>
        </div>
        <div className="modal-body">
          <p>Tên buổi học: <span className="tx-medium">{LessionName || ''}</span></p>
