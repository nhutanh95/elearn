...
const initialState = {
	...
	loadOption: false,
	CountryName: 'Bach Khoa University',
	StateName: 'Bach Khoa University',
};
			...
			>
				...
				<div className="upload-container">
					<label className="upload-avatar">
						...
						<img
							src={
								props?.getValues('avatar') ?? '../assets/img/default-avatar.png'
							}
							alt="avatar"
...
function TeacherInformation() {
	...
	const setMultipleValue = dataObj => {
		...
		for (let property in dataObj) {
			if (property === 'location') {
				setValue(property, dataObj[property].LocationName);
			} else if (property === 'state') {
				setValue(property, dataObj[property].StateName);
			} else {
	...
	// console.log('state.locationOptions ===============================');
	// console.log(state.locationOptions);
	...
	const _onSubmitInformation = async (data, e) => {
		...
		try {
			const res = await updateTeacherInfoProfile({
				...
				Location: data?.CountryName ?? '', // str
				State: data?.StateName ?? '', // str
			});
			res.Code === 1 &&
			...
			<form onSubmit={handleSubmitInformation(_onSubmitInformation)}>
				...
				<div className="teacher-info mg-l-0-f mg-t-30">
					<h5 className="mg-b-20">
						<i className="fas fa-user mg-r-5"></i>Basic Information
					</h5>
						...
						<div className="form-group col-12 col-sm-6 col-lg-3">
							<div className="input-float">
								...
								<input
									type="text"
									className={`form-control ${
										!!errors && errors.CountryName ? 'error-form' : ''
									}`}
						...
						<div className="form-group col-12 col-sm-6 col-lg-3">
							<div className="input-float">
								...
								<input
									type="text"
									className={`form-control ${
										!!errors && errors.StateName ? 'error-form' : ''
									}`}
								...
								<Controller
									as={
										<Select
											...
											menuPortalTarget={document.body}
											className={`${
												!!errors && !!errors.levelOfPurpose ? 'error-form' : ''
											}`}
										/>
									}
									control={control}
					...
					<h5 className="mg-b-20">
						<i className="fas fa-user-graduate mg-r-5"></i>Education Attainment
					</h5>
					<div className="row group-float-label">
						<div className="form-group col-12 col-sm-6">
							<div className="input-float">
