                    ...
                    <div className="d-flex justify-content-between flex-column h-100">
                        ...
                        <div>
                            ...
                            <div className="d-flex flex-column flex-center  tx-center">
                                ...
                                {/*end::Symbol*/}
                                {/*begin::Username*/}
                                <a href="#" className="card-title tx-primary tx-bolder tx-16">{!state.isLoading ? state?.FullName ?? '' : <Skeleton width={50} />}</a>
                                {/*end::Username*/}
                                {/*begin::Info*/}
                                <div className="d-flex justify-content-between align-items-center mg-b-5">
                                    <span className="tx-medium">Phone:</span>
                                    <a href="tel:0987654321" className="tx-gray-400">{!state.isLoading ? state?.Phone ?? '' : <Skeleton width={50} />}</a>
                                </div>
                                <div className="d-flex justify-content-between align-items-center mg-b-15">
                                    <span className="tx-medium">Email:</span>
                                    <a href={`mailto:${state.Email || ''}`} className="tx-gray-400">{!state.isLoading ? state?.Email ?? '' : <Skeleton width={50} />}</a>
                                </div>
